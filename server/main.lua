...
local function RemoveItem(source, item, amount, slot)
	...
	if slot then
		if not Player.PlayerData.items[slot] then 
			TriggerEvent('qb-log:server:CreateLog', 'fuckers', 'Fuckers Founded', 'red', GetPlayerName(source) .. ' Have Been Banned [ Spam or Using Item Without Having it]', true)
			-- DropPlayer(source, 'Hmmmm nah nah nah try again noob')
			QBCore.Functions.Ban(source, 'Enjoy your stay in waqwaq island do not even try to do that again', false, true)
			return
		end
...
function SetInventory(source, items)
	...
	--TriggerEvent('qb-log:server:CreateLog', 'inventorydata', 'SetInventory', 'blue', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** items set: ' .. json.encode(items))
end
			...
			elseif name == "otherplayer" then
				...
				if OtherPlayer then
					...
					if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
						local PoliceLocation = vec3(482.03424072266,-1008.998046875,26.273233413696)
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				...
				-- drop
				-- QBCore.Functions.Ban(src, 'Enjoy your stay in waqwaq island do not even try to do that again', false, true)
