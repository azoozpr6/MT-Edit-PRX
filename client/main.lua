...
local function OpenVending2()
    local ShopItems = {}
    ShopItems.label = "Vending Machine"
    ShopItems.items = Config.VendingSnaksItem
    ShopItems.slots = #Config.VendingSnaksItem
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "Vendingshop_"..math.random(1, 99), ShopItems)
end
...
RegisterNUICallback("GiveItem", function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('qb-vehiclekeys:server:GetClosestPlayer', function(ClosestPlayer)
        if #ClosestPlayer > 0 then 
            local menu = {}
            local itName = data.item.name
            if data.item.label then 
                itName = data.item.label
            end
            ...
            QBCore.Functions.Notify('No one here', 'error')
        end
    end)
    cb('ok')
end)
...
CreateThread(function()
    ...
    exports['qb-target']:AddTargetModel(trashes, {
        options = {
            {
